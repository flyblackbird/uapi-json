...
var _HotelsErrors2 = _interopRequireDefault(_HotelsErrors);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const HotelsParsingError = _HotelsErrors2.default.HotelsParsingError,
      HotelsRuntimeError = _HotelsErrors2.default.HotelsRuntimeError;
...
const getMedia = (version, rate) => {
  if (rate[`common_${version}:MediaItem`]) {
    return rate[`common_${version}:MediaItem`].filter(elem => elem.type !== 'Gallery').map(elem => elem);
  }
  return [];
};
...
const searchParse = function searchParse(rsp) {
  const self = this;
  const result = {};
      ...
      agregator.RateDetail = rateDetails.map(detail => {
        ...
        return {
          ...
          GuaranteeInfo: {
            ...
            amount: guar['hotel:DepositAmount'] ? Utils.price(guar['hotel:DepositAmount'].Amount) : null
          },
...
const bookParse = function bookParse(rsp) {
  const result = {};
  const self = this;
...
const cancelBookParse = function cancelBookParse(obj) {
  const result = {};
