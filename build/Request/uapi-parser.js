...
const parseString = xml => new Promise((resolve, reject) => {
  _xml2js2.default.parseString(xml, (err, json) => {
    if (err) {
      reject(err);
      return;
    }
...
function mergeLeaf(item) {
  ...
  return _lodash2.default.extend(item, leaf);
}
...
function defaultConfig(ver) {
  ...
  'common_' + ver + ':SSR', 'air:AirReservation', 'air:PassengerType', 'air:TicketInfo', 'air:Ticket', 'air:Coupon', 'air:AirExchangeBundle', 'common_' + ver + ':ResponseMessage', 'common_' + ver + ':BookingTraveler', 'air:BaggageAllowanceInfo', 'air:CarryOnAllowanceInfo', 'hotel:RateInfo', 'hotel:HotelSearchResult',
  // 'hotel:Amenities',
  'hotel:Amenity', 'hotel:HotelDetailItem', 'hotel:AggregatorHotelDetails', 'common_' + ver + ':MediaItem', 'util:CurrencyConversion', 'common_' + ver + ':TaxDetail', 'common_' + ver + ':SupplierLocator', 'passive:PassiveReservation', 'passive:PassiveSegment', 'passive:PassiveRemark', `common_${ver}:Email`, 'air:ExchangedTicketInfo', 'air:AirAvailabilityErrorInfo', 'air:AirSegmentError', `common_${ver}:GeneralRemark`, 'air:AirAvailabilityErrorInfo', 'air:AirSegmentError', `common_${ver}:Endorsement`];
  ...
  const fullCollapseListObj = ['air:ETR', 'air:FareTicketDesignator', `common_${ver}:Address`, `common_${ver}:ShippingAddress`, `common_${ver}:PhoneNumber`, `common_${ver}:ProviderReservationInfoRef`, // TODO check if can be collapsed
  ...
  */
  ...
  const CollapseKeysOnly = ['air:FlightDetailsRef', // can be a list of several tech stops
  'air:AirSegmentRef',
  /* 'air:FareInfoRef',*/
  'common_' + ver + ':BookingTravelerRef', 'common_' + ver + ':ProviderReservationInfoRef'];

  return {
    noCollapseList: noCollapseList,
    fullCollapseListObj: fullCollapseListObj,
    fullCollapseSingleKeyedObj: fullCollapseSingleKeyedObj,
...
Parser.prototype.mapArrayKeys = function mapArrayKeys(array, name) {
  ...
  if (!hasAllKeys) {
    _lodash2.default.forEach(array, (value, key) => {
      array[key] = self.mergeLeafRecursive(value, name /* + ':' + key*/);
    });
    return array;
  }

  const object = _lodash2.default.mapKeys(array, getItemEmbKey);
...
Parser.prototype.mergeLeafRecursive = function (obj, name) {
  ...
  if (_lodash2.default.isArray(obj)) {
    ...
    if (_lodash2.default.size(obj) === 1) {
      if (obj[0][listName]) {
        ...
        if (this.config.noCollapseList.indexOf(name) === -1 && !getItemEmbKey(obj[0]) || this.config.fullCollapseSingleKeyedObj.indexOf(name) !== -1) {
...
Parser.prototype.parseXML = function parseXML(xml) {
  return parseString(xml).then(res => {
    ...
    throw new _RequestErrors.RequestSoapError.SoapParsingError(res);
  }, err => {
    throw new _RequestErrors.RequestSoapError.SoapServerError(null, err);
  });
};

Parser.prototype.parseVersion = function (obj) {
  const detail = obj['SOAP:Fault'][0].detail[0];
  const parsedVersions = Object.keys(detail).filter(key => key.match(/ErrorInfo$/i)).reduce((acc, key) => acc.concat(Object.keys(detail[key][0].$).filter(detailKey => detailKey.match(/^xmlns/i)).map(detailKey => detail[key][0].$[detailKey])), []).map(xmlns => xmlns.match(/_(v\d+_\d+)$/)).filter(version => version !== null);
  ...
  return this.parseXML(xml).then(obj => {
    ...
    if (obj['SOAP:Fault']) {
      try {
        ...
        throw new _RequestErrors.RequestRuntimeError.VersionParsingError(obj, e);
      }
      return obj;
    }
    ...
    if (!data[self.rootObject]) {
      throw new _RequestErrors.RequestSoapError.SoapParsingError(obj);
    }
    ...
    throw new _RequestErrors.RequestSoapError.SoapParsingError(null, err);
  });
};
